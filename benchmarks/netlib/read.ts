...
const readBenchmarks = () =>
  readIndex()
  .filter(c => c.rows * c.cols <= 6_400_000) // solver is not yet intended for problems larger than this
...
export const read = (): Benchmark[] => readBenchmarks().filter(b => ok.includes(b.name))
